...
class DigestTransform extends stream.Transform {
  get actual() {
    return this._actual;
  }
...
class ProgressCallbackTransform extends stream.Transform {
  ...
  constructor(body, options) {
    ...
    _defineProperty(this, "download", async (fileOut, onProgress, validateOptions) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        fileOut.once('finish', () => {
          if (fileOut instanceof fs.WriteStream && typeof fileOut.close === 'function') {
            fileOut.close();
          }
  ...
  constructor(_options) {
    ...
    _defineProperty(this, "send", async () => {
      ...
      const writeToRequest = () => {
        if (requestBody === null) {
          clientRequest.end();
        } else if (requestBody instanceof stream.Stream) {
          requestBody.pipe(new stream.PassThrough()).pipe(clientRequest);
        } else {
      ...
      const bindRequestEvent = (onFulfilled, onRejected) => {
        ...
        clientRequest.on("response", res => {
          ...
          if (isRedirect(statusCode) && followRedirect) {
            ...
            this.options.parsedURL = new url.URL(String(headers.get("location")), parsedURL.toString());
            onFulfilled(this.send());
          }

          const responseBody = stream.pipeline(res, new stream.PassThrough(), error => {
            if (error !== null) {
              onRejected(error);
            }
          });
    ...
    _defineProperty(this, "send", () => {
      ...
      const bindRequestEvent = (onFulfilled, onRejected) => {
        ...
        clientRequest.on("response", res => {
          ...
          let responseBody = stream.pipeline(res, new stream.PassThrough(), pumpCallback);
          responseBody.on("cancel-request", cancelRequest);
          ...
          if (method !== "HEAD" && codings !== null && statusCode !== 204 && statusCode !== 304) {
            switch (codings) {
              case "br":
                responseBody = stream.pipeline(responseBody, zlib__default["default"].createBrotliDecompress(), pumpCallback);
                break;

              case "gzip":
              case `x-${"gzip"}`:
                responseBody = stream.pipeline(responseBody, zlib__default["default"].createGunzip(), pumpCallback);
                break;

              case "deflate":
              case `x-${"deflate"}`:
                stream.pipeline(res, new stream.PassThrough(), pumpCallback).once('data', chunk => {
                  if ((chunk[0] & 0x0f) === 0x08) {
                    responseBody = stream.pipeline(responseBody, zlib__default["default"].createInflate(), pumpCallback);
                  } else {
                    responseBody = stream.pipeline(responseBody, zlib__default["default"].createInflateRaw(), pumpCallback);
                  }
